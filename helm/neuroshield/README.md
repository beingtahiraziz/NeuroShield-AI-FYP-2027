# NeuroShield AI SOC — Helm chart

Production-style Helm chart for the NeuroShield AI SOC platform. Ships the backend API
...
|---|---|---|---|
| `neuroshield-backend` | Deployment | 2 (default) | FastAPI API + bundled SPA on port 6987 |
| `neuroshield-daemon` | StatefulSet | **1 (singleton)** | Autonomous orchestrator; webhook=8081, metrics=9090, health=9091 |
| `neuroshield-llm-worker` | Deployment | 2 (default) | ARQ worker for Claude requests off Redis queue |
| `neuroshield-postgres` | StatefulSet | 1 | Opt-out via `postgresql.enabled=false` |
| `neuroshield-redis` | StatefulSet | 1 | Opt-out via `redis.enabled=false` |
| `neuroshield-db-init` | Job (Helm hook) | 1 per install/upgrade | Applies `database/init/*.sql` idempotently |
...
```yaml
secrets:
  existingSecret: neuroshield-secrets
```
...
```bash
helm upgrade neuroshield ./helm/neuroshield -n neuroshield --reuse-values
```
...
> helm upgrade neuroshield ./helm/neuroshield -n neuroshield --reset-then-reuse-values
>
> # Or pass an explicit values file so the new defaults aren't lost
> helm upgrade neuroshield ./helm/neuroshield -n neuroshield -f my-values.yaml
> ```
>
...
- **Daemon probes**: target port `9091` (`/health`), not `9090`. Port `9090`
  ...
  `config.NEUROSHIELD_OTEL_ENABLED=true`.
...
```bash
# Lint
helm lint helm/neuroshield
helm lint helm/neuroshield -f helm/neuroshield/values-dev.yaml

# Render without applying
helm template neuroshield helm/neuroshield \
  --set secrets.anthropicApiKey=test \
  --set secrets.postgresPassword=test

# Dry-run install
helm install --dry-run --debug neuroshield helm/neuroshield \
  --set secrets.anthropicApiKey=test \
  --set secrets.postgresPassword=test
```
...
`helm/neuroshield/files/database-init/`. CI (`.github/workflows/helm-chart.yml`) will
fail on drift. To sync after adding new init SQL:

```bash
cp database/init/*.sql helm/neuroshield/files/database-init/
